...
load_dotenv(dotenv_path="C:/Users/Breti/AI_Workspace/.env")
...
class PDF2MDConverter:
    ...
    def query_deepseek(self, text_content):
        """Sendet Textblöcke zur semantischen Textglättung an das konfigurierte LLM (inkl. Chunking und Retries)."""
        if not self.api_key:
            print("Warnung: API-Key nicht gefunden. Führe Standard-Konvertierung aus.")
            return text_content
            
        # Wenn der Text sehr groß ist, splitten wir in Häppchen auf (ca. 30.000 Zeichen pro Aufruf)
        chunk_size = 30000
        chunks = []
        if len(text_content) > chunk_size:
            # Aufteilung nach Absätzen
        ...
        for idx, chunk in enumerate(chunks):
            ...
            success = False
            # Bis zu 3 Retries bei Netzwerkfehlern/Timeouts
            for attempt in range(1, 4):
                try:
                    response = requests.post(self.api_url, json=data, headers=headers, timeout=120)
                    if response.status_code == 200:
                        result = response.json()
                        refined_chunks.append(result["choices"][0]["message"]["content"])
                        success = True
                        break
                    else:
                        print(f"Versuch {attempt}: Fehler bei LLM-API ({response.status_code}): {response.text}")
                except Exception as e:
                    print(f"Versuch {attempt}: API-Aufruf fehlgeschlagen: {e}")
                
                if attempt < 3:
                    time.sleep(2 * attempt) # Exponentielles Backoff

            if not success:
                print(f"Geringere Qualität durch Fallback auf rohe Text-Häppchen {idx+1}!")
                    ...
                    if re.match(r'^\d+(\.\d+)*\s+\w', line_strip):
                        ...
                        if not is_toc_line:
                            ...
                            if match:
                                ...
                                # Entferne Seitenzahl am Ende, falls die Überschrift aus einem TOC ohne Punkte stammt (z. B. "Ausgangslage 3")
            ...
            for page_idx, page in enumerate(pdf.pages):
                ...
                for line in page_lines:
                    ...
                    if all_lines:
                        ...
                        is_prev_heading = re.match(r'^\d+(\.\d+)*\s+[A-Z\u00c0-\u00d6]', last_line)
                        ends_punc = last_line and last_line[-1] in ".!?\":"
                        
                        if not ends_punc and not is_struct and not is_prev_heading and not last_line.startswith("__TABLE_START__") and not last_line.startswith("#"):
            ...
            for line_idx, line in enumerate(all_lines):
                ...
                if re.match(r'^\d+(\.\d+)*\s+[A-Z\u00c0-\u00d6]', line_strip):
                    ...
                    if is_toc_line:
                        ...
                        # Prüfe, ob Text hinter der eigentlichen Überschrift angehängt wurde
                        match_num = re.match(r'^(\d+(?:\.\d+)*)', line_strip)
                        heading_text = line_strip
                        trailing_text = ""
                        
                        if match_num and match_num.group(1) in headings_map:
                            full_heading = headings_map[match_num.group(1)]
                            # Wenn der eingelesene Text länger als die bekannte Überschrift ist, trennen wir
                            if len(line_strip) > len(full_heading) + 3:
                                heading_text = full_heading
                                trailing_text = line_strip[len(full_heading):].strip()
                        
                        level = min(6, dots + 2) # 1 -> ##, 1.1 -> ###
                        formatted_blocks.append(f"{'#' * level} {heading_text}")
                        if trailing_text:
                            current_paragraph = [trailing_text]
                    in_list = False
                ...
                # Normaler Text (Fließtext)
                    ...
                    if current_paragraph:
                        ...
                        is_prev_heading = re.match(r'^\d+(\.\d+)*\s+[A-Z\u00c0-\u00d6]', prev_line)
                        
                        # Ein neuer Absatz startet, wenn die vorherige Zeile mit einem Satzzeichen endete
                        # ODER wenn die vorherige Zeile eine Überschrift war.
                        if (ends_punctuation or is_prev_heading) and starts_upper:
                            ...
                            # Außer die vorherige Zeile war eine Überschrift!
                            if is_prev_heading:
                                formatted_blocks.append(" ".join(current_paragraph))
                                current_paragraph = [line_strip]
                            else:
